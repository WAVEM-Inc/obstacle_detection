...
void ObsDetection::scan_callback(const std::shared_ptr<LidarMSG> scan){
	...
	int lp,lp_x,lp_y;
	float area_size_x1, area_size_x2, area_size_y1, area_size_y2;
	...
	if(distance_x < 0)
	{
		...
		area_size_x2=0.27;
		area_size_y1=0.0;
		area_size_y2=lidar_range - 0.0;
	}
	else
	{
		area_size_x1=-0.27;
		area_size_x2=0.27;
		area_size_y1=-lidar_range+0.0;
		area_size_y2=0.0;
	}
	...
	{
		if((!std::isnan(scan->ranges[lp])) && (scan->ranges[lp] > 0.019 && ((scan->ranges[lp]-scan->ranges[lp-1]) < 0.1 )))
		{
	...
	robot_detect_x=(DETECT_SIZE + (float)(area_size_x1))*DETECT_RES;
	robot_detect_y=(DETECT_SIZE + (float)(area_size_y1))*DETECT_RES;
	for(lp_y=0;lp_y < DETECT_RES*(area_size_y2-area_size_y1);lp_y++)
	{
		for(lp_x=0;lp_x < DETECT_RES*(area_size_x2-area_size_x1);lp_x++)
		{
